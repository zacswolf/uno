            ...
            if card.type >= Type.CHANGECOLOR:
                for c in range(Color.WILD):
                    ...
                    if card.type == Type.DRAW4:
                        temp_card_count[1:]+=4
                        temp_state = self.state_space.get_state(temp_hand, temp_card, temp_card_count)
                        temp_next_card_values.append(self.value.get_value(temp_state))
                    else:
                        temp_state = self.state_space.get_state(temp_hand, temp_card, temp_card_count)
                        temp_state0 = self.state_space.get_state(temp_hand, temp_card, temp_card_count0)
                        temp_state1 = self.state_space.get_state(temp_hand, temp_card, temp_card_count1)
                        ...
                        chance_of_playable_card = 83/108
                        chance_opp_has_card = 1-(chance_of_playable_card**card_counts[1]) 
                        temp_next_card_values.append(chance_opp_has_card*temp_value1 + (1-chance_opp_has_card)*chance_of_playable_card*temp_value0 + (1-chance_opp_has_card)*(1-chance_of_playable_card)*temp_value)
                ...
                chance_of_playable_card = 83/108
                chance_opp_has_card = 1-(chance_of_playable_card**card_counts[1]) 
                next_card_values.append(chance_opp_has_card*temp_value1 + (1-chance_opp_has_card)*chance_of_playable_card*temp_value0 + (1-chance_opp_has_card)*(1-chance_of_playable_card)*temp_value) 
                colors.append(card.color)
        # reward_prime = 0
